...
class RegistrationValidationSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['blacklist_domains'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('Blacklist e-mail domains'),
      '#description' => $this->t('Often times you will find specific domains getting past CAPTCHA modules, if they are installed. When attempting to register with any e-mail domains you enter below, the registration will be blocked and a message will appear informing the user to use a different e-mail address.'),
      '#collapsible' => TRUE,
      '#collapsed' => FALSE,
    ];
    $form['blacklist_domains']['domains'] = [
      '#type' => 'textarea',
      '#title' => $this->t('Blacklisted e-mail domains'),
      '#description' => $this->t('Specify blacklisted e-mail domains by using their domain name and top-level domain (.com, .org, .info, etc.). Enter one domain per line. Example domains are <em class="placeholder">example.com</em> for <em class="placeholder">@example.com</em> e-mail addresses and <em class="placeholder">example.*</em> for all <em class="placeholder">@example.*</em> e-mail addresses (.com, .org, .info, etc.).'),
    ...
    $form['blacklist_domains']['domains_message'] = [
      ...
      '#description' => $this->t('Enter the message to display on the page if the user\'s e-mail address domain is blacklisted. You can enter "%domain" (without quotes) anywhere in the message to print the domain name.<br>Default message: <em class="placeholder">The e-mail domain %domain has been blacklisted from registering. Please enter a different e-mail address.</em>'),
